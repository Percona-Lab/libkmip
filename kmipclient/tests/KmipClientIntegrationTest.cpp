...
#include "src/kmip_exceptions.hpp"
            ...
            for (const auto &key_id : created_key_ids)
              {
                ...
                try
                  {
                    auto res = kmip.client ().op_destroy (key_id);
                  }
  ...
  try
    {
      ...
      ASSERT_TRUE (destroy_result.empty ()) << "Destroy failed";
...
TEST_F (KmipClientIntegrationTest, GetNonExistentKey)
{
  ...
  try
    ...
    {
      FAIL () << "Failed to get non-existent key: " << e.what ();
    }
