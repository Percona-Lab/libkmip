...
KmipClient::op_locate_by_name (const name_t &name, enum object_type o_type) const
{
  ...
  RequestFactory::create_locate_by_name_rq (ctx, name, o_type, MAX_ITEMS_IN_SEARCH, 0);
  io->do_exchange (ctx);
  return rf.get_ids (0);
}

ids_t
KmipClient::op_locate_by_group (const name_t &group, enum object_type o_type) const
{
  ...
  do
    {
      RequestFactory::create_locate_by_group_rq (ctx, group, o_type, MAX_ITEMS_IN_SEARCH, offset);
      io->do_exchange (ctx);
...
ids_t
KmipClient::op_all (enum object_type o_type) const
{
  ...
  do
    {
      RequestFactory::create_locate_all_rq (ctx, o_type, MAX_ITEMS_IN_SEARCH, 0);
      io->do_exchange (ctx);
      auto exp = rf.get_ids (0);
      if (ids_t got = exp; !got.empty ())
        {
