...
public:
   ...
   */
  NetClient (const std::string host, const std::string port, std::string clientCertificateFn, const std::string clientKeyFn,
             const std::string serverCaCertFn, int timeout_ms) noexcept : m_host (host),
                                                                    m_port (port),
  ...
  /**
   * Closes SSL connection to the server, returns true if connection successful
   */
